        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm text-left">
                ...
                <tbody>
                    ...
                    <tr class="border-t"  onclick="document.getElementById('publisherDetailModal-{{ $publisher->publisher_id }}').showModal()">
                        <td class="px-4 py-2">{{ $loop->iteration }}</td>
                        <td class="px-4 py-2">{{ $publisher->publisher_name }}</td>
                        <td class="px-4 py-2">{{ $publisher->publisher_description }}</td>
                    </tr>
<dialog id="publisherDetailModal-{{ $publisher->publisher_id }}" class="modal">
    <div class="modal-box bg-white rounded-lg shadow-lg w-full max-w-xl fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" style="padding: 20px">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-lg text-gray-800">Update Publisher Form</h3>
            <form method="dialog">
                <button type="submit" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
            </form>
        </div>

        <form action="{{ route('admin.publisher.update', ['publisher_id' => $publisher->publisher_id]) }}" method="POST" class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <textarea
                    ...
                    class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >{{ $publisher->publisher_description }}</textarea>
...
<form action="{{ route('admin.publisher.delete', ['publisher_id' => $publisher->publisher_id]) }}" method="POST" class="mt-2 flex justify-end">
    @csrf
    @method('DELETE')
    <button type="submit"
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <h2 class="text-base font-semibold mb-2">Create Publisher Form</h2>
        <hr class="mb-4">
        <form action="{{ route('admin.publisher.store') }}" method="POST">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <textarea name="publisher_description" placeholder="Publisher Description" rows="3"
